...
export function toMasteryLookup(records) {
 ...
 */
export function meanMastery(record) {
    if (!record)
        return null;
 ...
 */
export function isConceptMastered(concept, mastery, fallbackThreshold = DEFAULT_MASTERY_THRESHOLD) {
 ...
 */
export function availableConcepts(concepts, mastery, threshold = DEFAULT_MASTERY_THRESHOLD) {
    const lookup = mastery instanceof Map ? mastery : toMasteryLookup(mastery);
    return concepts.filter((concept) => concept.requires.every((prerequisiteId) => {
...
export function buildCurriculumGraph(bundle) {
    ...
    for (const concept of bundle.concepts) {
        for (const requiredId of concept.requires) {
            ...
            if (bucket)
                ...
                dependents.set(requiredId, [concept]);
        }
    }
    ...
    for (const lesson of bundle.lessons) {
        for (const conceptId of lesson.conceptIds) {
            ...
            if (bucket)
                ...
                teachers.set(conceptId, [lesson]);
        }
    }
    const prerequisiteClosure = (id) => {
        const ordered = [];
 ...
 */
export function courseProgress(graph, courseId, mastery) {
    ...
    return course.unitIds.map((unitId) => {
        ...
        const status = conceptList.length > 0 && mastered.length === conceptList.length
            ? "completed"
            : touched
                ? "in-progress"
                : unlocked.size > 0
 ...
 */
export function nextLesson(graph, mastery) {
    const courses = [...graph.bundle.courses].sort((a, b) => levelRank(a.level) - levelRank(b.level));
        ...
        for (const unitId of course.unitIds) {
            ...
            for (const lessonId of unit.lessonIds) {
                ...
                if (conceptList.length === 0)
                    continue;
                const unlocked = new Set(availableConcepts(conceptList, mastery).map((c) => c.id));
                const pending = conceptList.find((concept) => unlocked.has(concept.id) && !isConceptMastered(concept, mastery));
                if (pending) {
 ...
 */
export function weakConcepts(graph, mastery, threshold = DEFAULT_MASTERY_THRESHOLD, limit = 10) {
    const weak = [];
    for (const record of mastery.values()) {
        ...
        weak.push({
            ...
            score: mean,
            weakSkills: weakestSkills(record).filter((skill) => (record.skills[skill] ?? 0) < (concept.masteryThresholds[skill] ?? threshold))
        });
    }
 ...
 * showing a padlock.
 */
export function learningPathTo(graph, targetConceptId, mastery) {
    const target = graph.concept(targetConceptId);
    if (!target)
        return [];
    const needed = new Set([targetConceptId, ...graph.prerequisiteClosure(targetConceptId)]);
    const order = graph.topologicalOrder();
    const ordered = order.length > 0 ? order : graph.bundle.concepts;
    return ordered.filter((concept) => needed.has(concept.id) && !isConceptMastered(concept, mastery));
}
/** Mean mastery per skill across every concept at `level`. Drives the progress screen. */
export function levelSkillProfile(graph, level, mastery) {
